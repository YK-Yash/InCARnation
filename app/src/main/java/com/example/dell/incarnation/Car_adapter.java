...
public class Car_adapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    List<car_available_model> data= Collections.emptyList();
    car_available_model current;
    int currentPos=0;

    // create constructor to innitilize context and data sent from MainActivity
    public Car_adapter(Context context, List<car_available_model> data){
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        car_available_model current=data.get(position);
        myHolder.textPlantName.setText(current.car_id);
        myHolder.textSize.setText(current.car_name);
        myHolder.textPrice.setText(current.kms);
